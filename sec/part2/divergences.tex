...
\end{equation}
and $\int \ccD k = \int \dd[D-3]{\vec{k}} \int \dd{k_+} \int \dd{r}$.
...
\subsubsection{Tensor Wave Function from Minkowski space}
...
\begin{equation}
  \begin{split}
    ...
    g^{\alpha\beta}\, g^{\gamma\delta}\,
    \Psi^{[2]}_{\qty[k_{\qty(3)},\, S_{\qty(3)}]\, \alpha\gamma}\qty(\qty[x])\,
    D_{\beta} \partial_{\delta} \Psi_{\qty[k_{\qty(2)}] }\qty(\qty[x])\,
    \Psi_{\qty[k_{\qty(1)}] }\qty(\qty[x])
    \\
    & \times
    \tr\qty( \liebraket{T_{\qty(1)}}{T_{\qty(2)}}_+\, T_{\qty(3)} ).
